...
OpenClaw には 1Password 連携などの Skills（プラグイン）があり、エージェントがローカルのツールやクレデンシャルにアクセスできる。便利な反面、セキュリティ的に不安が残る。まずは **サンドボックス環境で挙動を確認したかった**ので、Docker コンテナ内に閉じ込めて動かす構成を選んだ。
...
実際に動かしてみて感じたのは、ローカル LLM のハードルの高さだ。

- **遅い。** クラウド API と比べると体感で数倍〜10倍遅い。9B モデルでも返答に数十秒かかる場面がある
- **コンテキストサイズに余裕を持たせるとメモリ要求が跳ね上がる。** 128K コンテキストで ~12 GiB。32B モデルにすると数十 GiB。一般的なマシンでは厳しい
- **小さいモデルはシステムプロンプトを守らない。** エージェントとしての自律的な振る舞い（メモリ保存、ルール遵守）が弱い

### ただのチャットツールなら既成品で十分

ローカル LLM でチャットしたいだけなら、[Open WebUI](https://github.com/open-webui/open-webui) や [Jan](https://jan.ai/) などの既成ツールの方がセットアップも簡単で UI も洗練されている。OpenClaw をわざわざ構築する意味は、エージェントとしてのツール実行や自律的なタスク処理にある。

### Slack 連携ならもっとシンプルな方法がある

Slack と LLM を組み合わせたいだけなら、OpenClaw を経由せずとも：
- **Zapier / Make** などのインテグレーションツール
- **Cloud Run + Cloud Functions** で Slack Events API を受ける
- **Slack の Workflow Builder** + 外部 API 呼び出し

といった選択肢の方がシンプルで運用も楽だろう。

### OpenClaw がハマるユースケースは？

正直、まだ「これだ」というユースケースが見つかっていない。ただ、以下のような用途には向いていそうだと感じた：

- **定期的な Web サイト巡回・リサーチ** — HEARTBEAT.md でスケジュール定義し、エージェントが自律的に情報収集してメモリに記録する
- **長期間にわたるプロジェクトの文脈保持** — workspace の記憶機構により、セッションをまたいだコンテキストを維持できる
- **セキュリティが重要な環境での AI エージェント** — Docker サンドボックス内で動かし、外部アクセスを制限できる

逆に、単発の質問応答や簡単な自動化には明らかにオーバースペックだ。OpenClaw の真価は「エージェントに人格と記憶を持たせて、長期的に育てていく」という使い方にあると思う。今後、より大きなモデルやクラウド API と組み合わせたときに、本領を発揮するのかもしれない。
